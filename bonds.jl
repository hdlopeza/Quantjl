...
function ear2bey{ T <: FloatingPoint }( ear::T )
...
function bey2ear{ T <: FloatingPoint }( bey::T )
...
function ytm{ T <: FloatingPoint }( face_value, price, coupon_rate::T, n )
...
function price_from_ytm{ T <: FloatingPoint }( ytm::T, coupon_rate::T, n; face_value = 1000, pmt_type = 0 )
...
function dirty_price{ T <: FloatingPoint }( r::T, coupon_rate::T, n; face_value = 100.0, frac = 0.0 )
  validate_frac( frac )
  dp = 0.0
  coupon = face_value*coupon_rate/2
  for i = 1:n
...
function clean_price{ T <: FloatingPoint }( r::T, coupon_rate::T, n; face_value = 100.0, frac = 0.0 )
  validate_frac( frac )
...
#### Generate cashflow ####
function cf_from_bond{ T <: FloatingPoint }( face_value, coupon_rate::T, n )
  coupon_pmt = face_value*coupon_rate/2
...
function bond_duration{ T <: FloatingPoint }( r::Vector{T}, cf::Vector{T} )
...
function bond_duration{ T <: FloatingPoint }( r::T, cf::Vector{T} )
  n = length( cf )
  dcf, wcf = 0.0, 0.0  # discounted and weighted discounted cash flow
  for i=1:n
...
function macaulay_duration{ T <: FloatingPoint }( ytm::T, cf::Vector{T} )
...
function modified_duration{ T <: FloatingPoint }( macd::T, r::T )
  return macd/( 1 + r )
end
...
#### Calculate the linear approx. of the change in price from change in yield to maturity ####
function duration_from_pch{ T <: FloatingPoint }( pch::T, r0::T, r1::T )
  m_r0 = 1 + r0
  m_r1 = 1 + r1
...
function pctchange_from_duration{ T <: FloatingPoint }( duration::T, r0::T, r1::T )
